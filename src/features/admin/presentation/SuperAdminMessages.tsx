...
export function SuperAdminMessages() {
  ...
  const markUnread = async (id: string) => {
    ...
    await supabase.from('admin_messages').update({ is_read: false }).eq('id', id);
  };
  ...
  const tabs: { key: FilterTab; label: string; count?: number }[] = [
    ...
    { key: 'member',    label: 'Members',    count: messages.filter(m => m.role === 'member').length },
    ...
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ color: textColor, fontSize: '22px', fontWeight: 700, marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {unreadCount > 0 && (
              <span style={{
                background: '#fbbf24', color: '#0f172a', fontSize: '12px',
                fontWeight: 800, borderRadius: '999px', padding: '2px 10px',
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {[1, 2, 3].map(i => (
            <div key={i} className="min-card" style={{ padding: '24px', opacity: 0.4, height: '80px', borderRadius: '12px' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
            ...
            return (
              <div
                ...
                style={{
                  padding: '0',
                  borderRadius: '12px',
                  borderLeft: !m.is_read ? '3px solid #fbbf24' : '3px solid transparent',
                  ...
                  <div style={{ padding: '0 20px 20px', borderTop: `1px solid ${borderCol}` }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <button
                        ...
                        className="min-button"
                        style={{ padding: '8px 14px', borderRadius: '8px', border: 'none', color: '#ef4444', fontSize: '12px', fontWeight: 600, cursor: 'pointer', background: 'rgba(239,68,68,0.08)' }}
                      >
                        Delete
                      </button>
                    </div>
