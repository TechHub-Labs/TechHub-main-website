 ...
 * Manages state and orchestrates the 3-step form flow.
 * Rendering is delegated to JoinStep1, JoinStep2, JoinStep3.
 */
...
type FormData = { name: string; email: string; level: string; portfolio: string };
...
export function JoinUsPage() {
  ...
  const [form, setForm]     = useState<FormData>({ name: '', email: '', level: '', portfolio: '' });
  ...
  const transitionStep = (nextStep: 1 | 2 | 3, dir: 'forward' | 'back') => {
    setStepDir(dir);
    setAnimating(true);
    setTimeout(() => { setStep(nextStep); setAnimating(false); }, 260);
  };
  ...
  const handleNext = () => {
    if (validate1()) { setErrors({}); transitionStep(2, 'forward'); window.scrollTo(0, 0); }
  };
  ...
  // ── Derived styles (shared across steps) ─────────────────────────────────
          ...
          >
            ...
            {step === 2 && (
              <JoinStep2
                ...
                onBack={() => { transitionStep(1, 'back'); setErrors({}); }}
                onSubmit={handleSubmit}
              />
            )}

            {step === 3 && (
              <JoinStep3 colors={colors} cardBg={cardBg} borderColor={borderColor} />
            )}
          </div>
        </PageMargin>
      </main>
