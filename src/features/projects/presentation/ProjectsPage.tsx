 ...
 * Handles filtering, pagination and delegates row rendering to <ProjectRow>.
 */
      ...
      <main className="w-full flex-1 overflow-x-hidden">
        <PageMargin>
          ...
          <section className="relative z-50 mb-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: colors.textMuted, flexShrink: 0 }}>
              ...
              <div className="relative">
                ...
                {showCatDrop && (
                  <div
                    ...
                    style={{ background: colors.bgCard, borderColor: colors.divider, animation: 'popIn 0.2s cubic-bezier(0.34,1.56,0.64,1) both' }}
                  >
                    {categories.map(c => (
            ...
            <div className="relative">
              <button
                ...
                style={{ background: colors.bgCard, borderColor: colors.divider, color: colors.text }}
              >
              ...
              {showStatusDrop && (
                ...
                >
                  {statuses.map(s => (
                    <button key={s} onClick={() => { setStatusFilter(s); setShowStatusDrop(false); }}
                      className="w-full px-4 py-2 text-left text-sm transition-opacity hover:opacity-70"
                      style={{ color: colors.text }}
                    >{s}</button>
                  ))}
          ...
          {/* Project list */}
          <section className="overflow-hidden rounded-3xl border" style={{ borderColor: colors.divider }}>
            {paginated.map((project, index) => (
              <ProjectRow key={project.id} project={project} index={index} colors={colors} />
            ))}
          </section>

          {/* Pagination */}
          <section className="flex flex-col gap-6 py-10 sm:flex-row sm:items-center sm:justify-between sm:pb-24">
            <p className="text-sm sm:text-base" style={{ color: colors.textMuted }}>Page {page} of {totalPages}</p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                <button
                  ...
                  style={{ background: page === n ? colors.tagBg : 'transparent', color: colors.text, transform: page === n ? 'scale(1.1)' : 'scale(1)' }}
                >{n}</button>
              ))}
              {page < totalPages && (
                <button
                  ...
                  style={{ background: colors.bgCard, color: colors.text, borderColor: colors.divider }}
                >Next</button>
              )}
            </div>
          </section>
