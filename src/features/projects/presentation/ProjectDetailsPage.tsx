  ...
  const project = useMemo(() => {
    if (dbProject) {
      return {
        ...
        status:
          dbProject.status ||
          (dbProject.in_development ? "IN DEVELOPMENT" : "LIVE"),
        category: dbProject.category || "Product",
        teamSize: dbProject.team_size || "Nil",
        tech: Array.isArray(dbProject.tech)
          ? dbProject.tech.join(", ")
          : dbProject.tech || "",
        launchDate: dbProject.launch_date || "TBA",
        website: dbProject.live_url || "",
        tags: Array.isArray(dbProject.tech)
          ? dbProject.tech
          : dbProject.tech
            ? dbProject.tech.split(",").map((t: string) => t.trim())
            : [],
              ...
              >
                ...
                >
                  <p className="whitespace-pre-wrap">
                    {project.about || "Detailed description coming soon."}
                  </p>

                  {project.about2 && (
                    <p className="whitespace-pre-wrap">{project.about2}</p>
                  )}
                </div>
              </div>
