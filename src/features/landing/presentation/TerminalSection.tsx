/**
 * TERMINAL SECTION — Speed Code Game CTA
 * ─────────────────────────────────────────────────────────────────────────────
 * - SectionTitle with green curtain reveal
 * - Terminal window slides in from below
 * - Animated "typing" on the terminal prompt
 * - Play button with ripple on click
 */
...
export function TerminalSection({ colors }: TerminalSectionProps) {
  ...
  // Trigger terminal typing when section enters view
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setTermVisible(true);
          // Stagger each terminal line appearing
              ...
              <div className="flex gap-2">
                {['#ff5f57', '#febc2e', '#28c840'].map((c, i) => (
                  <span
                    ...
                    style={{
                      background: c,
                      animation: i === 2 ? 'pulseGlow 2s ease-in-out infinite' : 'none',
                    }}
                  />
                ))}
              </div>
                ...
                <Link to="/play" onClick={handlePlay}>
                  <button
                    ...
                    style={{
                      background: '#A3D045',
                      color: '#0F1524',
                      animation: termVisible ? 'pulseGlow 3s ease-in-out infinite' : 'none',
                      transition: 'transform 0.3s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.3s ease',
                    }}
                    onMouseEnter={e => {
                      (e.currentTarget as HTMLButtonElement).style.transform = 'scale(1.07)';
                      (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 12px 40px rgba(163,208,69,0.5)';
                    }}
                    onMouseLeave={e => {
                  ...
                  >
                    ...
                    {rippling && (
                      <span
                        ...
                        style={{ animation: 'rippleExpand 0.6s ease-out both', background: 'rgba(255,255,255,0.3)' }}
                      />
                    )}
                    ▶ &nbsp;Play Now
                  </button>
